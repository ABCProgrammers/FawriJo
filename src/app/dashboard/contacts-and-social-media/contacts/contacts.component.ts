...
import { forkJoin, takeUntil, Subject, of, catchError } from 'rxjs';
...
export class ContactsComponent {
    ...
    return this.fb.group(
      {
        ...
        contactCountry: [data?.contactCountry || null, [Validators.required]],
        contactMethod: [data?.contactMethod || null, [Validators.required]],
        contactDetails: [data?.contactDetails || null, [Validators.required]],
        add: [false],
        edit: [false],
  ...
  getLookups() {
    ...
    const contactMethod$ = this._httpService.get(`${this._httpService.apiUrl.Lookup.GetLookups}?lookupTypeId=63&status=1001&pageSize=1000`).pipe(catchError(error => of(error)));
    forkJoin([country$, contactMethod$]).pipe(takeUntil(this.destroy$)).subscribe((response) => {
      this.countryList = response[0].data;
      this.contactMethodList = response[1].data;
      this.getDataList();
    });
  }
  getDataList(params?) {
    let APIURL = `${this._httpService.apiUrl.ContactsSM.GetContacts}?`;
    ...
    if (this.dataList.length) {
      this.dataList.forEach(item => {
        let obj = {
          ...
          contactCountry: item?.contactsCountry ? item?.contactsCountry[0]?.lookupID : null,
          contactMethod: item?.contactMethod?.lookupID,
  ...
  saveData(row: FormGroup) {
    ...
    const formData = this._httpService._helperService.convertFormGroupToFormData(row, ['status', 'edit', 'add', 'contactID']);
    let URL = this._httpService.apiUrl.ContactsSM.AddContacts;
    formData.append('status', value?.status ? '1001' : '1002');
    if (!value.edit) {
      URL = this._httpService.apiUrl.ContactsSM.AddContacts;
    }
    else {
      URL = this._httpService.apiUrl.ContactsSM.EditContacts;
      formData.append('contactsID', value?.contactID);
    }
  ...
  deleteRow(row, index) {
    ...
    formData.append('contactsID', row.value?.contactID);
    this._httpService.post(`${this._httpService.apiUrl.ContactsSM.DeleteContacts}`, formData).pipe(takeUntil(this.destroy$)).subscribe({
      next: response => {
        if (response.isSuccess) {
  ...
  checkHasRowPendingChanges() {
    let isEdited = this.f.value.contacts.some(x => x.add || x.edit);
    if (isEdited) {
